...
template <>
QGaussJacobi<0>::QGaussJacobi (const unsigned int ,
				const double ,
				const double)
  :
  // there are n_q^dim == 1
...
Quadrature<1> (n)
{
  ...
  long double a,b,c,p1,p2,p3,pp,temp,z(0.);
...
//TODO: prendere un'altra versione, questa è scandalosa (e protetta)
  for (unsigned int i=0;i<n;++i)
  {
      ...
      x[i] = z;
      this->quadrature_points[n-i-1] = Point<1>(.5+.5*static_cast<double>(z));
      this->weights[n-i-1]=.5*exp(gammln(alf+n)+gammln(bet+n)-gammln(n+1.0)-
	      gammln(n+alfbet+1.0))*temp*pow(2.0,alfbet)/(pp*p2);
  }
}

template <int dim>
QGaussJacobi<dim>::QGaussJacobi (const unsigned int n, const double alf, const double bet)
